...
contract CrowdFund {
    ...
    function createCampaign(string memory cid) external {
        ...
        campaigns[campaign_id] = Campaign(msg.sender, cid, true, campaignState.STARTED);
    ...
    function getCampaignList(address _sender) public view returns (Campaign[] memory) {
        ID[] memory ids = campaignList[_sender];
    ...
    function sendAmount(uint256 _campaignId) external payable {
